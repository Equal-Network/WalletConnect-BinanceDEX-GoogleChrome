...
chrome.runtime.onMessage.addListener(function (message, _, callBack) {
  ...
  if (message && message.showModalQrCodde) {
    var button = document.getElementById("Unlock_Wallet_Connect");
    if (button) {
      button.click();
      callBack();
    }
  ...
  if (message && message.showNotification) {
    ...
    var _button = document.getElementById("Unlock_Wallet_Connect");
    if (_button) {
      _button.click();
    }
  ...
  if (message && message.checkQrModalBox) {
    var qrBox1 = document.getElementsByClassName("gebWbn");
    if (qrBox1.length > 0) {
      callBack(true);
      return true;
    }
    var qrBox2 = document.getElementsByClassName("TmNzu");
    if (qrBox2.length > 0) {
      callBack(true);
      return true;
    }
